...
The language model proposes structured candidate records from source documents. Candidates that satisfy all source-grounding, schema, semantic, and safety checks may be published automatically. Ambiguous or higher-risk cases are held for human review; invalid evidence is rejected. Unsupported optional quantities may be omitted, but uncertainty about the event, entity, attribution or action cannot be dropped to salvage a claim.

Deterministic extraction remains active. The new multilingual extractor is configured on, initially in shadow mode with model publication disabled; credentials are still required. Shadow output stays private and does not change public records. Guarded mode independently reconstructs supported relationships using the narrow tested semantic adapter and then applies the existing publication validator. An exact quotation is necessary but is not, by itself, proof of a proposed relationship. Broad Bengali/English structures outside the tested grammar remain review exceptions, not automatically established facts.

Frozen source revisions preserve ordered passages and original text. The application resolves model-proposed quotes using exact, NFC Unicode or whitespace-normalized matching, mapping offsets back to original text. No fuzzy match or translation proves evidence. Original and normalized values remain separate. Context, negation, correction, dates, quantity attribution, duplicates and source permissions are checked independently. A publication date is not silently converted into an event date. Relationships proposed by a model remain suggestions until validated.

Provider/model/task/schema versions, source revision, usage and validation decisions are retained privately; any model-assisted published record carries provenance and a short original evidence locator. There is no website chatbot, public Q&A, model training, fine-tuning, critic model, VLM, OCR, face recognition or local model installation. Remote extraction sends bounded public source text to the configured provider; review its terms and data-use settings first. Model failure is separate from source failure and cannot suspend a record. Technical limits and promotion criteria are documented in docs/LLM_EXTRACTION.md.
