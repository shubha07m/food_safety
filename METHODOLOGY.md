...
Deterministic extraction remains active alongside the multilingual extractor. The extractor is configured on and credentials are required; model-derived candidates are eligible for publication only after the existing independent publication validator passes. An exact quotation is necessary but is not, by itself, proof of a proposed relationship. Broad Bengali/English structures outside the tested grammar remain review exceptions, not automatically established facts.
