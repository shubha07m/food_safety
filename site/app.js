...
try {
  ...
  $('dataset-version').textContent = `${data.schema_version} / pipeline ${data.pipeline_version}`; $('run-status').textContent = `Last run result: ${status.last_run_result || 'not reported'}. Published: ${data.record_count}; held from publication: ${status.pending_count ?? 'not reported'}.`;
