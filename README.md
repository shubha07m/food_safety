...
From `food_safety`, the local conda environment named `food` lives at `.conda/envs/food`. Activate it by path to avoid modifying conda's global environment registry:

```bash
conda activate food_safety/.conda/envs/food
