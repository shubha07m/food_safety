...
TASK_VERSION = "puja_pandal_extraction_v1"
...
def discover(root: Path, source_id=None, fetcher=None):
    ...
    if fetcher is None:
        ...
        fetcher = Fetcher(policies, pipeline_settings(root))
    results = []
...
def extract(root: Path, source_id=None, max_calls=None, extractor=None):
    ...
    results, calls, input_tokens, output_tokens = [], 0, 0, 0
    for path in paths:
        ...
        try:
            ...
            input_tokens += int(reply.get("input_tokens", 0))
            output_tokens += int(reply.get("output_tokens", 0))
